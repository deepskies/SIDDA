...
def train_SIDDA(
    ...
    for epoch in range(epochs):
        ...
        for i, (batch, target_batch) in tqdm(
            ...
            if epoch < warmup:
                ...
                DA_loss = None  # No domain loss during warmup
            else:
        ...
        train_DA_losses.append(train_domain_loss)
        ...
        if (epoch + 1) % report_interval == 0:
            ...
            with torch.no_grad():
                for i, (batch, target_batch) in enumerate(
                    ...
                    if epoch >= warmup:
                        val_domain_loss += (
                            domain_loss_.item()
                        )  # Accumulate domain loss only after warmup
            ...
            if epoch >= warmup:
                val_domain_loss /= len(
                    val_dataloader
                )  # Normalize domain loss only if accumulated

            val_losses.append(val_loss)
            val_classification_losses.append(val_classification_loss)
            val_DA_losses.append(val_domain_loss)
    ...
    plt.plot(steps, train_DA_losses, label="Train Domain Loss")
    ...
    plt.plot(validation_steps, val_DA_losses, label="Validation Domain Loss")
    plt.xlabel("Epochs")
    plt.ylabel("Loss")
